...
class UserModel {
    ...
    public function setVerified(int $id, bool $v): void {
        $this->db->prepare("UPDATE users SET is_verified = ? WHERE id = ?")
            ->execute([$v ? 1 : 0, $id]);
    }

    public function incFailed(int $id): void {
        $this->db->prepare("UPDATE users SET failed_login_attempts = failed_login_attempts + 1 WHERE id = ?")
            ->execute([$id]);
        $u = $this->findById($id);
        if ($u && (int) $u['failed_login_attempts'] >= MAX_LOGIN_ATTEMPTS) {
            $this->lock($id);
        }
    }
